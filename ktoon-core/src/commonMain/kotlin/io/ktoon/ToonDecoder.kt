...
internal class ToonDecoder(
    ...
    private fun throwTypeMismatch(
        ...
        throw SerializationException(
            "Type mismatch at line $lineNumber: cannot decode '$value' as" +
                " $expectedType$causeMessage\n\nContext:\n$context"
        )
    }
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder {
        return when (descriptor.kind) {
            ...
            StructureKind.CLASS, StructureKind.OBJECT, kotlinx.serialization.descriptors.PolymorphicKind.SEALED -> {
                ...
                while (lexer.hasMoreLines() && lexer.lines[lexer.currentLineIndex].trim()
                        .isEmpty()
                ) {
                    lexer.advanceLine()
                }
        ...
        if (!lexer.hasMoreLines()) {
            ...
            throw SerializationException(
                "Expected row data at line $lineNumber but reached end of input. " +
                    "Table header declared ${headerInfo.size} rows but only " +
                    "$actualRowCount were found\n\nContext:\n$context"
            )
        }
...
internal class ToonRowDecoder(
        ...
        if (rowValues.size != fieldNames.size) {
            ...
            throw SerializationException(
                "Row field count mismatch at line $rowLineNumber: " +
                    "row has ${rowValues.size} values but header declares ${fieldNames.size}" +
    ...
    private fun getCurrentValue(): String {
        ...
        if (value.isEmpty()) {
            ...
            val context = getContextSnippet(rowLineNumber - 1)
            throw SerializationException(
                "Cannot decode empty value for field '$fieldName'" +
                    " as non-nullable type at line $rowLineNumber\n\nContext:\n$context"
            )
        }
        return value
    }
    ...
    private fun throwTypeMismatch(
        ...
        val causeMessage = if (cause != null) "\nCause: ${cause.message}" else ""
        throw SerializationException(
            "Type mismatch at line $rowLineNumber, field '$fieldName': cannot decode '$value' " +
                "as $expectedType$causeMessage\n\nContext:\n$context"
        )
    }
